...
## 15. 收起按钮 / 细条窗口（2026-09-25 新增）
- [ ] 版本页"导入整合包"左侧有"收起"按钮，点击后窗口缩成细条（约 440×78）
- [ ] 导入整合包弹窗底栏"取消/导入中..."左侧同样有"收起"按钮，导入进行中可收起
- [ ] 收起后导入/下载在后台继续，进度不中断
- [ ] 细条内容：左侧展开按钮 ⤢、当前选择（版本名，未选时"-"）、右侧启动游戏按钮
- [ ] 细条"启动游戏"状态与主按钮一致（禁用灰条/可点击/运行中变停止方块/出错警告）
- [ ] 点展开按钮恢复原窗口大小**与位置**，界面完整（打开中的导入弹窗与进度仍在）
- [ ] 收起/展开连点无异常（幂等，geometry 只保存一次）
- [ ] 收起期间游戏状态变化（安装完成、启动、退出）无报错，展开后按钮状态正确
- [ ] 导入完成时处于收起状态 → 展开后能看到结果提示，弹窗状态正常
